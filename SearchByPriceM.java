...
public class SearchByPriceM {
	...
	public SearchByPriceM(String c_id, String c_name, String passw){
...
try{
			...
			System.out.println("Connection Successfull");
			Statement stmt = conn.createStatement();
		    String query = "Select * from product where unitPrice > "+from.getText()+" and unitPrice < " + to.getText();
	...
	public SearchByPriceM(){
		...
		search.addActionListener(e->{
			...
			try{
			    ...
			    Vector row = new Vector();
			    int b=1;
			   // if(rs.equals(false)){b=1;}
			    
			   /* if(rs.next()){
					JOptionPane.showMessageDialog(null,"No product found plz try again with right input");
					j.setVisible(false);
				   new	SearchByPriceM();
			    }
			    */
